...
def upsampling(vallabel, valpssm, valhot):
    ...
    val_pssm_temp = np.zeros( (valpssm.shape[0], valpssm.shape[1], valpssm.shape[2]) )
    val_pssm = np.zeros((location_size, valpssm.shape[2]))
    val_label_temp = np.zeros( (vallabel.shape[0], vallabel.shape[1], vallabel.shape[2]) )
    ...
    #    Q8_length = 0
        ...
        for j in range(valhot.shape[1]):
            # check whether inside Noseq part
            if np.sum(valhot[i,j,:]) != 0:
                # check whether current class is minor class
                if np.argmax(vallabel[i,j,:]) in minor_set:
                    val_hot_temp[i,sequence_end] = int(np.argmax(valhot[i,j,:]))
                    val_pssm_temp[i, sequence_end, :] = np.copy(valpssm[i, j, :])
                    val_label_temp[i, sequence_end, :] = np.copy(vallabel[i,j, :])
                    sequence_end += 1
            else:
                break
        ...
        val_pssm[index: index+sequence_end,:] = np.copy(val_pssm_temp[i, :sequence_end, :])
        val_label[index: index+sequence_end,:] = np.copy(val_label_temp[i, :sequence_end, :])
        index += sequence_end
    ...
    # shape (218, 700, 21)
    val_pssm_upsampling = np.zeros( (num_sequence+1, valpssm.shape[1], valpssm.shape[2]) )
    # shape (218, 700)
    val_hot_upsampling = np.zeros( (num_sequence+1, valhot.shape[1]) )
    # shape (218, 700, 8)
    val_label_upsampling = np.zeros( (num_sequence+1, vallabel.shape[1], 4) )#vallabel.shape[2]) )
    ...
    y_res_one_hot = convert_to_one_hot(y_res, 4)
    ...
    # append the last row of sequence
    start = 0
    for times in range(700//remain):
        val_pssm_upsampling[-1, start:start+remain, :] = np.copy( X_res[index:, 1:] )
        val_hot_upsampling[-1, start:start+remain] =  X_res[index:, 0] 
        val_label_upsampling[-1, start:start+remain, :] = np.copy( y_res_one_hot[index:, :] )
        start += remain
        
    remain = 700%remain
    val_pssm_upsampling[-1, start:remain+start, :] = np.copy( X_res[index:index+remain, 1:] )
    val_hot_upsampling[-1, start:remain+start] =  X_res[index:index+remain, 0] 
    val_label_upsampling[-1, start:remain+start, :] = np.copy( y_res_one_hot[index:index+remain, :] )    
...
def load_cb513():
    ...
    return test_hot, testpssm, test_label
"""
def load_casp10():
