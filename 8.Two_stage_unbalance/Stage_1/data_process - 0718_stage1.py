...
def upsampling(vallabel, valpssm, valhot):
    ...
    for i in range(y_val.shape[0]):
        if y_val[i] == 1 or  y_val[i] == 3 or  y_val[i] == 6:
             y_val[i] = 4
        elif  y_val[i] == 5:
             y_val[i] = 1
        elif  y_val[i] == 7:
    ...
    # shape (218, 700, 8)
    val_label_upsampling = np.zeros( (num_sequence, vallabel.shape[1], 5) )#vallabel.shape[2]) )
    ...
    y_res_one_hot = convert_to_one_hot(y_res, 5)
...
def load_cb513():
    ...
    label_4 = np.array([0,0,0,0,1])
    label_3 = np.array([0,0,0,1,0])
    label_1 = np.array([0,1,0,0,0])
    ...
    for i in range(testhot.shape[0]):
        for j in range(testhot.shape[1]):
            if np.sum(testhot[i,j,:]) != 0:
                ...
                if np.argmax(testlabel[i,j,:]) == 1 or  np.argmax(testlabel[i,j,:]) == 3 or np.argmax(testlabel[i,j,:]) == 6:
                     test_label[i,j] = np.copy(label_4)
                elif  np.argmax(testlabel[i,j,:]) == 5:
                     test_label[i,j] = np.copy(label_1)
                elif  np.argmax(testlabel[i,j,:]) == 7:
                     test_label[i,j] = np.copy(label_3)
